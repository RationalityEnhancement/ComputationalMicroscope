...
def getMapping(dataclip, subject, timeline = None):
    ...
    if timeline is None:
        #print(dataclip['datastring']['questiondata']['global_timeline'])
        #timeline = eval(json.loads(dataclip['datastring'])['questiondata']['global_timeline'])
        timeline = dataclip['datastring']['questiondata']['global_timeline']
    ...
    for trial in dataclip['datastring']['data']:
...
def json2dict(data):
    """
    Turn a json database file into a list
    """
...
def getParticipantInfoExp(dataclip, subject):

    # retrieve main infos on first level of dictionary
    begin_hit = dataclip['beginhit']
    begin_exp = dataclip['beginexp']
    end_hit = dataclip['endhit']
    worker_id = dataclip['workerid']
    hit_id = dataclip['hitid']
    assignment_id = dataclip['assignmentid']
    condition = dataclip['cond']
    datastring = dataclip['datastring']
    status = dataclip['status']
    total_time = '-'
    ...
    if datastring is None:
        ...
        #condition_type = datastring['questiondata']['condition_type'] #todo: where is this? how is this used?
        condition = int(datastring['condition'])

        # loop thorugh all trials and categorize
        for trial in datastring['data']: #todo: what does this chunk do?
            trial_key = reduceNodeString(trial['trialdata']['internal_node_id']) #000
            ML_test_code = ['0410', '0411', '0412', '0413', '0414','0415','0416','0417','0418','0419', '000'] #todo: waht are those numbers?
            ...
            # --------  shared -----
            # quiz attempts
            if trial_key in ['011']: #todo: what are those numbers?
                attempts_quiz += 1
    ...
    return {
        ...
        'trialTimeMean': round(np.mean(trial_time),1),
        ...
        'score_exp_total': round(np.sum(scores_exp),4),
...
# ----- IMPORT
# import data as csv loaded from postgre database
# with open('data/dataclipsExp.json', 'r') as handle:
#     data = json.load(handle)

data = pd.read_csv("data/dataclipExp.csv")

data_dict = json2dict(data)
#index_cond0 = 1 # index of data_dict for a full expererimental trial
...
#pretty_print(getParticipantInfoExp(data_dict, index_cond0))
#pretty_print(getParticipantInfoExp(data_dict, index_cond1))
#print('')


# ----- CREATE dataframe
dataframe = makeDataframe(data_dict, getParticipantInfoExp)
print(len(dataframe))
print('')

# ---- REMOVE Early Quitter
dataframe = dataframe[dataframe.status != 4].reset_index(drop=True, inplace=False)
print(len(dataframe))
print('')

# ---- computes delta to reach min wage for every one
#delta_bonus = getDeltaBonus(df, base_pay, min_wage_ph, max_bonus= 1000, total_bonus = False):
...
# --- export bonus for boto3_script.py
exportBonus(dataframe, total_bonus = True)
dataframe.to_csv('data/dataframe.csv')

# anonymised:
dataframe.drop(columns=['WorkerId', 'hitId', 'assignmentId','datastring']).to_csv('data/dataframe.csv', index_label='pid')